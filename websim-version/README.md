...
but the AI takes a lot of structure as a given.
This behavior is actually *useful* when giving it your own code to make one-off changes, but problematic when the AI is building repeatedly on top of its own output.
...
- `function makeStermFall` (even computers make typos; should be "stem" not "sterm")
- `if (lastBallPositions.length > 30) { // 1/2 second at 60 FPS` sure, that math checks out, but this is frame-rate dependent, 60 FPS is not guaranteed...
...
- `#score, #ballsLeft, #level, #multiplier` and `#restartButton, #nextLevelButton, #resetBallButton, #debugToggle` in CSS instead of classes (it started with IDs, so it kept using them even though it became more and more verbose)
- `// Catcher (formerly target)` didn't really need the note about the name change, I just wanted the name to be changed
...
- I didn't really want the stems to disintegrate, I just wanted them to fall
- disintegrated stems still collide, so you can actually run into a situation where the ball rests on them, getting stuck (if a bunch of them fall on a stem and then the ball falls on the fragments, it fails to trigger the ball rest logic, because there's no (unbroken) stem directly colliding with the ball for it to break)
